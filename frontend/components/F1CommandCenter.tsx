...
export default async function F1CommandCenter({ season = 2024, round = 4 }: F1CommandCenterProps = {}) {
  const fallback = season === FALLBACK_DASHBOARD.season && round === FALLBACK_DASHBOARD.round
    ? FALLBACK_DASHBOARD
    : { ...FALLBACK_DASHBOARD, season, round };
  const dashboard = (await getF1Dashboard(season, round)) ?? fallback;
